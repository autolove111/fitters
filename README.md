...
<!-- FITTERS_AI_COACH_SECTION_START -->
## 🏋️ AI 私人健身顾问 — 个性化健身 RAG

AI 私人健身顾问把 Fitters 中已经记录的运动、睡眠、饮食和目标数据，与权威健身知识库结合，为不同身体情况生成有依据、可追溯的训练计划。它不只给出一套固定模板，还会参考用户的健身目标、训练水平、伤病限制、可用器械、偏好时间和近期完成情况，解释为什么这样安排。

### 完整 RAG 流程

1. **知识接入与清洗**：读取带来源信息的 Markdown 健身资料，校验机构、标题、网址、权威等级和正文，清理无关标记后按语义句子切分，并保留适量上下文重叠。
2. **增量向量索引**：使用文档哈希识别新增或修改内容，只重建发生变化的资料；知识文件被删除后会同步清理旧索引，并按 Embedding 模型和维度隔离数据。
3. **混合召回**：同时执行向量检索与关键词检索，再通过倒数排名融合汇总候选内容，兼顾语义相似度和健身术语的准确命中。
4. **个性化重排序**：综合目标、伤病、器械、来源权威性和检索得分重新排序，并限制单一来源占用过多位置，让最终依据更相关也更有代表性。
5. **受约束生成**：模型只能使用召回到的知识片段生成热身、主训练和放松计划；输出经过结构校验，引用必须对应真实片段，重复或伪造引用会被移除。
6. **安全降级**：当用户填写伤病或身体限制时，系统会补充低冲击和停止训练条件；Embedding、检索或模型接口不可用时，仍会返回保守的兜底计划，不影响健康数据记录功能。

### 个性化数据闭环

- 用户健身画像：年龄、身高体重、目标、训练水平、伤病限制、器械和偏好训练时间。
- 近期健康记录：Free 使用较短历史窗口和较少引用；Pro 使用更长历史、趋势分析和更丰富的依据。
- 计划留痕：每次生成请求、结果和引用都会保存到计划历史中，同时记录会员额度使用情况。
- 可观测与评估：内部状态接口可查看索引文档数、分块数、Embedding 模型和维度；离线案例覆盖减脂、增肌、膝部不适、恢复和家庭训练，并计算 Hit@K、MRR 与来源覆盖率。

### 使用方式

1. 使用 `demo / demo123` 登录 Fitters。
2. 进入运动模块并打开 `AI Personalized Fitness Plan`。
3. 完善 Fitness Profile，选择 Free 或 Pro，填写当天需求后生成计划。
4. 查看训练步骤、个性化理由、风险提示、知识引用和剩余额度。

后端对外接口为 `POST /api/plans/personalized-workout`；AI 服务内部接口为 `POST /plans/personalized-workout`，并通过内部密钥限制直接访问。RAG 索引状态可通过受保护的 `GET /rag/status` 查询。
<!-- FITTERS_AI_COACH_SECTION_END -->
