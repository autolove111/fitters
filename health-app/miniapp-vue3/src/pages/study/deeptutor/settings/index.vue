<template>
  <view class="settings-page">
    ...
    <view class="settings-group">
      <view class="setting-item" @click="go('/pages/study/deeptutor/settings/status')">
        <u-icon name="info-circle" size="36" color="#4f46e5" />
        <text>System Status</text>
        <u-icon name="arrow-right" size="28" color="#d1d5db" />
      </view>

      <view class="setting-item" @click="go('/pages/study/deeptutor/settings/llm')">
        <u-icon name="setting" size="36" color="#10b981" />
        <text>Model Catalog</text>
        <u-icon name="arrow-right" size="28" color="#d1d5db" />
      </view>

      <view class="setting-item" @click="go('/pages/study/deeptutor/settings/tools')">
        <u-icon name="list" size="36" color="#f59e0b" />
        <text>Tools</text>
        <u-icon name="arrow-right" size="28" color="#d1d5db" />
      </view>

      <view class="setting-item" @click="go('/pages/study/deeptutor/settings/appearance')">
        <u-icon name="photo" size="36" color="#8b5cf6" />
        <text>Appearance</text>
        <u-icon name="arrow-right" size="28" color="#d1d5db" />
      </view>

      <view class="setting-item" @click="go('/pages/study/deeptutor/history/list')">
        <u-icon name="clock" size="36" color="#6366f1" />
        <text>Chat History</text>
        <u-icon name="arrow-right" size="28" color="#d1d5db" />
      </view>

      <view v-if="userStore.state.isAdmin" class="setting-item" @click="go('/pages/study/deeptutor/settings/users')">
