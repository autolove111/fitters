...
【调用链路位置】ChatAgent.retrieve_context() → tool_registry.execute("rag") → rag_search()

薄封装层，调用 RAGService.search() 执行向量检索并生成答案。
kb_name 由用户在前端选择，此处不处理别名或默认值。
"""
...
async def rag_search(
    ...
    **kwargs,
    ...
    if kb_base_dir is None:
        from aidlearning.multi_user.knowledge_access import resolve_for_rag
