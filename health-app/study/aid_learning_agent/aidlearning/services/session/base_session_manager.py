#!/usr/bin/env python
"""
BaseSessionManager — 会话持久化基类

【调用链路位置】SessionManager 继承此类

提供统一的 JSON 文件存储格式，各模块（chat, solve, question 等）继承后
只需实现 _get_session_id_prefix() 等少量抽象方法。

存储路径通过 PathService 解析，支持多用户隔离。
"""
