"""
Turn Runtime Manager -- Turn-level runtime for unified chat streaming.

Core responsibility: manage the full lifecycle of a Turn (create, run, cancel, regenerate).

Call chain:
  unified_ws.py -> TurnRuntimeManager.start_turn() -> _run_turn()
    -> ContextBuilder -> ChatOrchestrator.handle()
      -> Capability.run() -> Agent + tool calls -> StreamEvent stream

Main features:
  - start_turn(): start a new turn, create background task
  - _run_turn(): core execution function (~600 lines), build context and run chat
  - subscribe_turn() / subscribe_session(): real-time event subscription
  - cancel_turn(): cancel a running turn
  - regenerate_last_turn(): regenerate the last reply
  - submit_user_reply(): handle user reply for ask_user tool

Event flow:
  _run_turn() -> _publish_live_event() -> subscriber.queue -> unified_ws.py -> client
"""

# === Python stdlib ===
from __future__ import annotations         # PEP 563 deferred annotation evaluation

import asyncio                             # async (Task, Queue, Lock)
from collections.abc import AsyncIterator, Callable, Sequence  # type hints
import contextlib                          # context manager utils (suppress)
from contextvars import Token              # contextvars token (LLM scope)
from dataclasses import dataclass, field   # dataclass
import json                                # JSON serialization (event mirroring)
import logging                             # logging
from typing import TYPE_CHECKING, Any, Literal  # type hints

# === project modules ===
from aidlearning.core.stream import StreamEvent, StreamEventType  # event types
from aidlearning.services.llm.utils import clean_thinking_tags    # clean LLM thinking tags
from aidlearning.services.path_service import get_path_service    # path service (workspace dir)
from aidlearning.services.session.protocol import SessionStoreProtocol  # session store protocol

if TYPE_CHECKING:
    from aidlearning.services.llm.config import LLMConfig  # LLM config (type check only)

logger = logging.getLogger(__name__)

# Memory reference types: recent, profile, scope, preferences, summary
...
def _should_capture_assistant_content(event: StreamEvent) -> bool:
    ...
    规则:
      - 必须是 CONTENT 类型事件
      - 如果没有 call_id(顶层 LLM 回复),直接捕获
      - 如果有 call_id,只捕获 call_kind == "llm_final_response" 的事件
        (避免捕获中间工具调用产生的内容)
    """
...
def _clip_text(value: str, limit: int = 4000) -> str:
    """截断文本到指定长度,超过时末尾添加 ...[truncated] 标记."""
...
_INTERRUPTED_TURN_ERROR = "Turn interrupted by server restart. Please retry your message."  # 服务器重启时的错误提示


def _sanitize_session_title(raw: str) -> str:
    """清理 LLM 生成的会话标题,去掉多余噪声.
...
def _extract_memory_references(payload: dict[str, Any]) -> list[MemoryReference]:
    """从请求 payload 中提取用户选择的记忆引用类型(L3 记忆槽位).
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    """将前端传入的 LLM 选择参数转为标准化字典 {profile_id, model_id}."""
    from aidlearning.services.model_selection import LLMSelection
...
def _request_snapshot_metadata(
    *,
    ...
    """构建请求快照元数据,保存用户消息时附带前端的完整上下文(模式,工具,附件等)."""
...
def _format_question_bank_entry(entry: dict[str, Any]) -> str:
    """将单条题库条目渲染为结构化 Markdown 文本,包含题目,选项,答案和解析."""
    lines: list[str] = []
...
async def _count_branch_user_turns(
    ...
    """计算当前分支祖先链上的用户消息数量,用于确定当前是第几轮对话.
...
async def _build_question_bank_context(
    ...
    """根据 entry_ids 从题库获取条目,渲染为上下文文本供 LLM 使用."""
...
def _normalize_filename_list(raw: dict[str, Any]) -> list[str]:
    """统一处理旧版单文件名和新版多文件名输入,返回清理后的文件名列表.
...
def _extract_followup_question_context(
    ...
    """从配置中提取测验追问上下文(题目,选项,答案,解析等),供后续对话使用."""
...
def _extract_persist_user_message(config: dict[str, Any] | None) -> bool:
    """从配置中提取是否持久化用户消息的标志(regenerate 时为 False,避免重复保存)."""
    if not isinstance(config, dict):
        return True
...
def _format_followup_question_context(context: dict[str, Any], language: str = "en") -> str:
    """将测验追问上下文格式化为 system prompt 文本,支持中英文."""
...
class _LiveSubscriber:
    """实时订阅者,持有自己的事件队列,用于接收 Turn 执行过程中的实时事件."""
    queue: asyncio.Queue[dict[str, Any]]


@dataclass
class _TurnExecution:
    """单个 Turn 的运行时状态,包含任务,订阅者,事件缓存和序列号."""
    turn_id: str                          # Turn 唯一标识
    session_id: str                       # 所属会话 ID
    capability: str                       # 对话模式(chat/deep_solve 等)
    payload: dict[str, Any]               # 完整的请求参数
    task: asyncio.Task[None] | None = None  # 后台执行任务
    subscribers: list[_LiveSubscriber] = field(default_factory=list)  # 实时订阅者列表
    events: list[dict[str, Any]] = field(default_factory=list)  # 已发布的事件缓存
    next_seq: int = 1                     # 下一个事件的序列号
    events_flushed: bool = False          # 事件是否已持久化到数据库


class TurnRuntimeManager:
    """Turn 运行时管理器 — 管理 Turn 的完整生命周期.

    职责:
      - 创建和启动 Turn(start_turn)
      - 管理后台执行任务(_run_turn)
      - 发布和缓存事件(_publish_live_event)
      - 提供实时事件订阅(subscribe_turn / subscribe_session)
      - 支持取消(cancel_turn)和重新生成(regenerate_last_turn)
      - 处理用户回复暂停/恢复(submit_user_reply)
    """
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        ...
        参数:
          store: 会话存储后端(默认从全局获取)
        """
        from aidlearning.services.session import get_session_store

        self.store = store or get_session_store()
        self._lock = asyncio.Lock()  # 保护 _executions 的并发访问
        self._executions: dict[str, _TurnExecution] = {}  # turn_id → 执行状态
        # ask_user 工具暂停时的用户回复队列(turn_id → Queue)
        self._reply_queues: dict[str, asyncio.Queue[dict[str, Any] | None]] = {}

    async def _has_live_execution(self, turn_id: str) -> bool:
        """检查该 Turn 是否仍在本进程中有活跃的执行任务."""
    ...
    async def _fail_orphan_running_turn(self, turn: dict[str, Any] | None) -> dict[str, Any] | None:
        """将服务器重启后遗留的"孤儿" running 状态 Turn 标记为 failed.
    ...
    async def _recover_orphan_running_turns_for_session(self, session_id: str) -> None:
        """在创建新 Turn 前,清理该 session 中所有遗留的孤儿 running 状态 Turn."""
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from aidlearning.capabilities.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from aidlearning.multi_user.model_access import apply_allowed_llm_selection
            ...
            from aidlearning.multi_user.model_access import redacted_model_access
        ...
        if llm_selection:
            from aidlearning.services.config import get_model_catalog_service
            from aidlearning.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from aidlearning.api.routers.settings import get_enabled_optional_tools
    ...
    async def regenerate_last_turn(
        ...
        流程:
          1. 检查是否有活跃 Turn(有则报 regenerate_busy)
          2. 获取最后一条用户消息(没有则报 nothing_to_regenerate)
          3. 删除最后一条助手消息
          4. 复用用户消息的参数,合并 overrides 覆盖
          5. 调用 start_turn() 创建新 Turn(_persist_user_message=False)
    ...
    async def cancel_turn(self, turn_id: str) -> bool:
        ...
        如果有活跃的 asyncio.Task, 直接 cancel();
        如果没有本地任务但数据库状态仍为 running, 更新为 cancelled.
        返回是否成功取消.
        """
        async with self._lock:
            execution = self._executions.get(turn_id)
    ...
    async def submit_user_reply(
        ...
        *,
        ...
        """提交用户对 ask_user 工具暂停的回复,唤醒等待中的 agentic loop.

        参数:
          text: 单条自由文本回复(旧版兼容)
          answers: 结构化回复列表 [{questionId, text}, ...](v2 多问题)

        返回 True 表示回复已接受,False 表示该 Turn 未在等待回复.
        """
    ...
    async def subscribe_turn(
        ...
        """订阅某个 Turn 的事件流(支持断点续传).

        流程:
          1. 从数据库加载历史事件(after_seq 之后的)
          2. 加入实时订阅者,接收内存中未发布的事件
          3. 如果 Turn 已结束,补充数据库中的遗漏事件
          4. 持续从队列接收实时事件,直到收到 None 哨兵(Turn 结束)
          5. 退出时清理订阅者,必要时合成 DONE 事件

        after_seq: 从第几个事件开始接收(用于断线重连跳过已收到的事件)
        """
    ...
    def _synthesize_done_event(turn_id: str, turn: dict[str, Any] | None) -> dict[str, Any]:
        """合成 DONE 事件 -- 当订阅结束但未收到实时 DONE 时,从数据库状态构建.
        ...
        nonetheless terminated server-side.
        """
        status = "completed"
        error: str | None = None
        if turn is not None:
    ...
    def _synthesize_error_event(turn_id: str, turn: dict[str, Any] | None) -> dict[str, Any] | None:
        """合成 ERROR 事件 — 从 failed 状态的 Turn 构建终端错误事件."""
    ...
    async def subscribe_session(
        ...
        自动查找该 Session 的活跃 Turn,然后委托给 subscribe_turn().
        如果没有活跃 Turn,直接返回(不产生任何事件).
        """
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        """Turn 的核心执行函数 — 构建上下文,执行对话,保存结果.

        这是整个对话系统的核心,约 600 行.流程:
          1. 解析请求参数(capability,config,attachments 等)
          2. 处理附件(上传到存储,提取文档文本)
          3. 构建各类上下文(notebook,history,question_bank,book,memory,skills)
          4. 持久化用户消息到数据库
          5. 组装 UnifiedContext
          6. 调用 ChatOrchestrator.handle() 执行对话
          7. 收集助手回复,保存到数据库
          8. 生成会话标题(首次对话时)
          9. 异常处理(CancelledError → cancelled,其他 → failed)
          10. 清理(重置 LLM 选择,通知订阅者,移除执行状态)
        """
        ...
        async def _wait_for_user_reply() -> dict[str, Any] | None:
            """ask_user 工具暂停时调用,等待用户通过 submit_user_reply 提交回复."""
            return await reply_queue.get()

        try:
            from aidlearning.agents.notebook import NotebookAnalysisAgent
            from aidlearning.core.context import Attachment, UnifiedContext
            from aidlearning.runtime.orchestrator import ChatOrchestrator
            ...
            try:
                from aidlearning.services.notebook import get_notebook_manager
            except ImportError:
                get_notebook_manager = None  # notebook service removed
            from aidlearning.memory.short_term.context_builder import ContextBuilder
            from aidlearning.memory.short_term.buffer_manager import get_buffer_manager
            from aidlearning.services.skill import get_skill_service
            ...
            from aidlearning.services.storage import get_attachment_store
            ...
            from aidlearning.utils.document_extractor import extract_documents_from_records
            ...
            # 激活用户选择的 LLM 模型(设置 contextvars 作用域)
            ...
            _buf_parent = branch_parent_id if branch_parent_explicit else None
            buffer_temp_id = session_buffer.append(
                role="user",
                content=raw_user_content,
                parent_id=_buf_parent,
            )

            # ── 自动压缩（如果窗口溢出）─────────────────────────────
            await session_buffer.maybe_compress()

            # ── 从 buffer 获取上下文（零 I/O）────────────────────────
            conversation_history = session_buffer.get_context()

            # 包装为 ContextBuildResult 兼容格式
            from aidlearning.memory.short_term.context_builder import (
                ContextBuildResult, build_history_text, count_tokens,
            )
            context_text = build_history_text(conversation_history)
            history_result = ContextBuildResult(
                conversation_history=conversation_history,
                conversation_summary=session_buffer.summary,
            ...
            if memory_references:
                memory_context = await memory_store.retrieve_memory(
            ...
            from aidlearning.multi_user.skill_access import assigned_skill_ids
            ...
            if is_chat_capability:
                from aidlearning.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
            ...
            # 组装 UnifiedContext — 包含对话所需的全部上下文信息
            ...
            # 执行对话 — 调用 ChatOrchestrator 路由到对应 Capability
            ...
            # 更新 buffer（供下轮 context 构建使用）
            session_buffer.append(
                role="assistant",
                content=assistant_content,
                parent_id=buffer_temp_id,
            )
            await session_buffer.maybe_compress()
            await buffer_mgr.flush(session_id, self.store)
    ...
    async def _publish_live_event(
        ...
        """发布实时事件 — 分配 seq,缓存,推送给所有订阅者.

        流程:
          1. 为事件分配递增的 seq 序列号
          2. 缓存到 execution.events(用于后续持久化)
          3. 推送给所有实时订阅者的队列
          4. 返回序列化后的事件字典
        """
    ...
    async def _maybe_generate_session_title(
        ...
        *,
        ...
        """为新会话生成 LLM 撰写的简短标题(仅在标题为 "New conversation" 时触发).
        ...
        has already filled in a title), it short-circuits.
        """
        if not session_id:
            return
        session = await self.store.get_session(session_id)
    ...
    def _mirror_event_to_workspace(execution: _TurnExecution, payload: dict[str, Any]) -> None:
        """将 Turn 事件镜像写入 workspace 的 events.jsonl 文件(用于调试和审计)."""
        try:
...
# 按 store 实例缓存的运行时管理器单例(不同数据库路径对应不同实例)
_runtime_instances: dict[str, TurnRuntimeManager] = {}


def get_turn_runtime_manager() -> TurnRuntimeManager:
    """获取 TurnRuntimeManager 单例(按 store 实例缓存).

    在 unified_ws.py 中通过此函数获取管理器实例,
    然后调用 start_turn() / cancel_turn() 等方法.
    """
    from aidlearning.services.session import get_session_store
