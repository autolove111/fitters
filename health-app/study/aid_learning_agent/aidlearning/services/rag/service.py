"""RAG Service — 知识库检索增强生成服务

【调用链路位置】rag_tool.py → RAGService.search() → LlamaIndex Pipeline

提供：
  - initialize(): 初始化知识库（文档导入、向量索引构建）
  - add_documents(): 追加文档
  - search(): 查询检索 + 答案生成
  - list_kbs(): 列出知识库
  - delete_kb(): 删除知识库

底层使用 LlamaIndex Pipeline（见 services/rag/pipelines/llamaindex/）。
"""
...
from aidlearning.runtime.home import get_runtime_data_root
...
class RAGService:
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from aidlearning.logging import capture_process_logs
