"""Provider Registry — LLM Provider 元数据注册表

【调用链路位置】factory.py → find_by_name/find_by_model → ProviderSpec

ProviderSpec 定义每个 Provider 的属性：
  - name/keywords: 名称和匹配关键词
  - backend: 使用哪个实现（openai_compat/anthropic/azure_openai 等）
  - default_api_base: 默认 API 地址
  - is_gateway/is_local/is_oauth: 类型标记
  - supports_prompt_caching: 是否支持 prompt 缓存

新增 Provider 只需在 PROVIDERS 列表中添加一个 ProviderSpec。
"""
