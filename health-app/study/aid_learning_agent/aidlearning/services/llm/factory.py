"""LLM Factory — 统一 LLM 调用工厂

【调用链路位置】BaseAgent.call_llm/stream_llm → factory.complete/stream

职责：
  - resolve_call_config(): 解析 LLM 配置（合并调用方参数与全局配置）
  - _resolve_provider_spec(): 根据 binding/model/url 查找 ProviderSpec
  - get_runtime_provider(): 实例化具体 Provider（OpenAI/Anthropic/Azure 等）
  - complete(): 非流式调用 → provider.chat_with_retry()
  - stream(): 流式调用 → provider.chat_stream_with_retry()，支持 think 标签、chunk 合并

Provider 路由优先级：显式 binding > gateway 检测 > model 匹配 > 本地服务器检测 > 默认 openai
"""
...
from aidlearning.config.settings import settings
...
async def complete(
    ...
    **kwargs: Any,
    ...
    调用链：resolve_call_config → get_runtime_provider → provider.chat_with_retry()
    返回完整响应文本。
    """
...
async def stream(
    ...
    **kwargs: Any,
    ...
    调用链：resolve_call_config → get_runtime_provider → provider.chat_stream_with_retry()
    内部通过 asyncio.Queue 将回调式流转为 AsyncGenerator。
    支持 <think>/</think> 标签透传、chunk 合并（减少网络往返）。
    """
    caller_extra_headers = kwargs.pop("extra_headers", None)
    reasoning_effort = kwargs.pop("reasoning_effort", None)
    image_data = kwargs.pop("image_data", None)
