"""
Multimodal Message Utilities — 多模态消息处理

【调用链路位置】base_agent.py / factory.py → prepare_multimodal_messages()

将纯文本消息 + 图片附件转换为 LLM 要求的多模态消息格式。
支持 OpenAI 兼容 API（image_url blocks）和 Anthropic API（image source blocks）。
不支持 vision 的模型会自动剥离图片。
"""
