"""Provider Factory — 根据 LLMConfig 实例化具体 Provider

【调用链路位置】factory.complete/stream → get_runtime_provider(config) → 具体 Provider

根据 config.provider_name 的 backend 字段选择：
  - openai_compat → OpenAICompatProvider（OpenAI 及兼容 API）
...
from aidlearning.services.llm.config import LLMConfig, get_llm_config
from aidlearning.services.llm.provider_core import (
    AnthropicProvider,
    AzureOpenAIProvider,
    GenerationSettings,
