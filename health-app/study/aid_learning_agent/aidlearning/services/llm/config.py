"""
LLM Configuration — LLM 配置管理

【调用链路位置】chat.py / BaseAgent / factory.py 均调用 get_llm_config()

配置来源：data/user/settings/model_catalog.json → resolve_llm_runtime_config()
提供 LLMConfig 数据类（model, api_key, base_url, binding 等）
支持 ContextVar 级别的配置覆盖（set_scoped_llm_config）
"""
...
from aidlearning.services.config import resolve_llm_runtime_config
from aidlearning.services.provider_registry import canonical_provider_name, find_by_name
...
def get_llm_config() -> LLMConfig:
    ...
    优先级：ContextVar 覆盖 > 全局缓存 > resolve_llm_runtime_config() 解析
    """
