...
from aidlearning.runtime.banner import labels_for, print_banner, resolve_language
from aidlearning.runtime.home import AIDLEARNING_HOME_ENV, PACKAGE_ROOT, get_runtime_home
...
def _reset_runtime_singletons() -> None:
    ...
    try:
        ...
        pass
    try:
        from aidlearning.services.config.runtime_settings import RuntimeSettingsService

        RuntimeSettingsService._instances.clear()
    except Exception:
        pass
    try:
        from aidlearning.services.config.model_catalog import ModelCatalogService
...
def _packaged_web_dir() -> Path | None:
    try:
        import aidlearning_web
    except ImportError:
        return None
    path = Path(aidlearning_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".aidlearning-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged AidLearning Web app.")
    ...
    raise SystemExit(
        "AidLearning Web assets are not installed. Install the full app with `pip install -U aidlearning`, "
...
def start(home: str | Path | None = None) -> None:
    ...
    os.environ[AIDLEARNING_HOME_ENV] = str(runtime_home)
    ...
    common_env[AIDLEARNING_HOME_ENV] = str(runtime_home)
