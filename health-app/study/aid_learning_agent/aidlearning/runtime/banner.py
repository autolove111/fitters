...
from aidlearning.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `aidlearning start` again."
        ),
        ...
        "start.port_in_use": (
            "AidLearning cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `aidlearning start`。"
        ),
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]AidLearning[/]",
