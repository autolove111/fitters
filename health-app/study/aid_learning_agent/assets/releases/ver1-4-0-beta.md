# AidLearning v1.4.0-beta Release Notes
...
maze and make `aidlearning start` / `aidlearning init` first-class.
...
- **`aidlearning/runtime/banner.py`** — localized startup banner shared
  between `aidlearning start` and `aidlearning init`; reads the language
  preference from interface settings so the banner matches the UI locale.
- **`init_wizard.py`** — interactive `aidlearning init` wizard with provider
...
- **`aidlearning start` is the recommended launcher** — `start_web.py` /
  `start_tour.py` continue to work but are now thin wrappers around the
  new `runtime/launcher.py`. Run `aidlearning init` once to seed providers
...
**Full Changelog**: https://github.com/HKUDS/AidLearning/compare/v1.3.10...v1.4.0-beta
