# AidLearning v1.4.0 Release Notes
...
isolation hardening, and the `aidlearning start` / `aidlearning init` launcher)
...
- **`build_provider_extra_kwargs`** in `aidlearning/core/agentic/client.py`
...
The `aidlearning chat` REPL surface was tightened so it matches the public
docs at [aidlearning.info](https://aidlearning.info).

- `aidlearning chat --config key=value` and `--config-json '<json>'` seed
...
- `aidlearning plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `AidLearning.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `aidlearning …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U aidlearning`
...
**Full Changelog**: https://github.com/HKUDS/AidLearning/compare/v1.4.0-beta...v1.4.0
