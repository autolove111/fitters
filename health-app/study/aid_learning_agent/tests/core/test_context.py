...
from aidlearning.core.errors import (
    ConfigurationError,
    AidLearningError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = AidLearningError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = AidLearningError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, AidLearningError)
        assert issubclass(ValidationError, AidLearningError)
        assert issubclass(ServiceError, AidLearningError)
