...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("aidlearning.api.routers.question", None)
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("aidlearning.agents")
    fake_agents_question = types.ModuleType("aidlearning.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "aidlearning.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "aidlearning.agents.question", fake_agents_question)

    fake_logging = _package("aidlearning.logging")
    ...
    monkeypatch.setitem(sys.modules, "aidlearning.logging", fake_logging)

    fake_config = types.ModuleType("aidlearning.services.config")
    fake_config.PROJECT_ROOT = Path.cwd()
    fake_config.load_config_with_main = lambda *_args, **_kwargs: {}
    monkeypatch.setitem(sys.modules, "aidlearning.services.config", fake_config)

    fake_llm_package = _package("aidlearning.services.llm")
    fake_llm_config = types.ModuleType("aidlearning.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "aidlearning.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "aidlearning.services.llm.config", fake_llm_config)

    fake_settings_package = _package("aidlearning.services.settings")
    fake_interface_settings = types.ModuleType("aidlearning.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "aidlearning.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "aidlearning.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("aidlearning.tools")
    fake_tools_question = types.ModuleType("aidlearning.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "aidlearning.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "aidlearning.tools.question", fake_tools_question)

    return importlib.import_module("aidlearning.api.routers.question")
