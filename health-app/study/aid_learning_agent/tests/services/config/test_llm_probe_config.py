...
from aidlearning.services.config.loader import get_agent_params
...
class TestLlmProbeUsesAgentsYaml:
    ...
    async def test_probe_passes_configured_max_tokens(self, monkeypatch):
        from aidlearning.services import llm as llm_module
        from aidlearning.services.config import test_runner as test_runner_module
        from aidlearning.services.config.test_runner import ConfigTestRunner, TestRun
...
def _real_get_token_limit_kwargs(model: str, max_tokens: int) -> dict[str, int]:
    ...
    from aidlearning.services.llm.config import uses_max_completion_tokens
...
async def _stub_context_window_detection(*_args, **_kwargs):
    from aidlearning.services.config.context_window_detection import (
