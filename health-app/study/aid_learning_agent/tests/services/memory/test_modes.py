...
from aidlearning.memory.shared import paths as paths_mod
from aidlearning.memory.consolidator.modes import audit as audit_mod
from aidlearning.memory.consolidator.modes import dedup as dedup_mod
from aidlearning.memory.consolidator.modes import update as update_mod
...
async def test_update_l2_appends_facts_from_chunk(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "aidlearning.memory.consolidator.modes.update.snap.read_snapshot",
    ...
    with (
        patch("aidlearning.memory.consolidator.modes.update.call_llm", side_effect=fake_llm),
...
async def test_update_l2_idempotent_when_no_new_entities(memory_dir, monkeypatch):
    ...
    with (
        patch(
            "aidlearning.memory.consolidator.modes.update.call_llm",
            side_effect=llm_returns_one,
        ),
        patch.object(update_mod, "load_memory_settings") as mock_settings,
    ):
        from aidlearning.memory.settings import DedupSettings, MemorySettings
...
async def test_audit_l2_applies_replace_edit(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "aidlearning.memory.consolidator.modes.audit.snap.read_snapshot",
    ...
    with patch("aidlearning.memory.consolidator.modes.audit.call_llm", side_effect=fake_llm):
...
async def test_dedup_early_stop_when_no_edits(memory_dir, monkeypatch):
    ...
    with (
        patch("aidlearning.memory.consolidator.modes.dedup.call_llm", side_effect=fake_llm),
