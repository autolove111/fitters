...
# === project modules ===
from deeptutor.core.stream import StreamEvent, StreamEventType  # event types
from deeptutor.services.llm.utils import clean_thinking_tags    # clean LLM thinking tags
from deeptutor.services.path_service import get_path_service    # path service (workspace dir)
from deeptutor.services.session.protocol import SessionStoreProtocol  # session store protocol

if TYPE_CHECKING:
    from deeptutor.services.llm.config import LLMConfig  # LLM config (type check only)
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            ...
            async def _emit_context_event(event: StreamEvent) -> None:
                """上下文构建过程中的事件回调(过滤掉 context 自身的事件)."""
