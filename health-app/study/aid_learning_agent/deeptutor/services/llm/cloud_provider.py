"""
Cloud LLM Provider — 云端 LLM 调用实现

【调用链路位置】factory.complete/stream → provider.chat_with_retry/chat_stream_with_retry

处理所有云端 API 调用（OpenAI、DeepSeek、Anthropic 等）。
使用 aiohttp 发送 HTTP 请求，支持流式 SSE 解析。
通过 build_chat_url() 构建请求 URL，build_auth_headers() 构建认证头。
"""
