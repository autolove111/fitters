...
【调用链路位置】factory.complete/stream 中 try/except → map_error()

将各 Provider 的特定异常（如 OpenAI 的 AuthenticationError、RateLimitError）
映射为统一的 LLM 异常类型（LLMAuthenticationError、LLMRateLimitError 等）。
"""
