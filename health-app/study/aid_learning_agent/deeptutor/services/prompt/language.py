"""Language Directive — 语言指令注入

【调用链路位置】ChatAgent.build_messages() → append_language_directive()

将"请使用 XX 语言回答"的指令注入 system prompt，确保 LLM 用用户选择的语言回复。
支持中/英/日/韩/法/德/俄/西/葡/意等语言。
"""
