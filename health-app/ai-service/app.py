...
from personalized_planner import build_personalized_workout_plan
    ...
    return jsonify(
        {
            ...
            "capabilities": ["today_workout_plan", "personalized_workout_plan"],
        }
    )
...
@app.post("/plans/personalized-workout")
def personalized_workout_plan():
    ...
    payload = request.get_json(silent=True) or {}
    try:
        plan = build_personalized_workout_plan(payload, authorization)
    except Exception as error:
        return jsonify({"code": 500, "message": str(error), "data": None}), 500
    return jsonify({"code": 0, "message": "ok", "data": plan})
